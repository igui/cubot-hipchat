...
class HipChat extends Adapter
  send: (user, strings...) ->
    if user.reply_to
      ...
      for str in strings
        @bot.message @rooms[user.room.toLowerCase()], str
    ...
    bot.onConnect =>
      ...
      if @options.rooms is "@All"
        bot.getRooms (err, rooms, stanza) ->
          if rooms
            for room in rooms
              # saves room for using later in send
              self.rooms[room.name.toLowerCase()] = room.jid
              console.log "Joining #{room.name}: #{room.jid}"
              bot.join room.jid
          else
